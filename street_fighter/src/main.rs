...
static RIGHT: GameAction = Right(PLAYER_STEP, 0.0);
static LEFT: GameAction = Left(-PLAYER_STEP, 0.0);
...
fn keyboard_update(
    ...
    if keyboard_input.pressed(KeyCode::Right) && keyboard_input.pressed(KeyCode::Up) {
        game_info.player_info.action = RIGHT.clone();
        ...
        if game_info.enemy_info.action == FIST || game_info.enemy_info.action == KICK {
            ...
            game_info.player_info.action = LEFT.clone();
        }
...
fn follow_logic(
    ...
    enemy_info.action = RIGHT.clone();
    if movement.x < 0.0 {
        sprite.flip_x = true;
        enemy_info.action = LEFT.clone();
    } else {
        sprite.flip_x = false;
        enemy_info.action = RIGHT.clone();
    }
...
fn setup_player_image(mut commands: &mut Commands, asset_server: &Res<AssetServer>, texture_atlases: &mut ResMut<Assets<TextureAtlas>>) {
    ...
    let mut transform = Transform::default();
    transform.translation = Vec3::new(-550.0, 350.0, 1.0);
...
fn setup_enemy_image(mut commands: &mut Commands, asset_server: &Res<AssetServer>, texture_atlases: &mut ResMut<Assets<TextureAtlas>>) {
    ...
    let mut transform = Transform::default();
    transform.translation = Vec3::new(400.0, 340.0, 1.0);
    transform.scale = Vec3::splat(1.0);
    image_spawn(&mut commands, atlas_handle, transform);
}
...
fn setup_player_life_bar(mut commands: &mut Commands) {
    setup_game_bar(&mut commands, Player, Color::rgb(0.219, 0.78, 0.74), -400.0, 320.0);
}

fn setup_enemy_life_bar(mut commands: &mut Commands) {
    setup_game_bar(&mut commands, Enemy, Color::rgb(0.219, 0.78, 0.74), 550.0, 320.0);
}
...
fn create_characters() -> HashMap<&'static str, [CharacterStats; 14]> {
    HashMap::from([
        ("ryu.png", [
            ...
            CharacterStats { action: RIGHT.clone(), x: 49.0, y: 104.0, column: 4, row: 1, offset: Vec2::new(202.5, 0.0) },
            CharacterStats { action: LEFT.clone(), x: 49.0, y: 104.0, column: 4, row: 1, offset: Vec2::new(202.5, 0.0) },
